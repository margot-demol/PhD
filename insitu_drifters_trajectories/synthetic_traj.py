...
def pos_vel_acc_spectral(df, dt, suf=""):
    # compute position
    df["x" + suf] = df.groupby("draw")["u" + suf].apply(
        pyn.geo.spectral_diff, dt, order=-1
    )
    df["y" + suf] = df.groupby("draw")["v" + suf].apply(
        pyn.geo.spectral_diff, dt, order=-1
    )
    # update velocities (diff cumulate_integrate vs differentiate do not give same result, numerical errors)
    df["u" + suf] = df.groupby("draw")["x" + suf].apply(
        pyn.geo.spectral_diff, dt, order=1
    )
    df["v" + suf] = df.groupby("draw")["y" + suf].apply(
        pyn.geo.spectral_diff, dt, order=1
    )
    df["U" + suf] = np.sqrt(df["u" + suf] ** 2 + df["v" + suf] ** 2)
    # compute acceleration
    df["ax" + suf] = df.groupby("draw")["x" + suf].apply(
        pyn.geo.spectral_diff, dt, order=2
    )
    df["ay" + suf] = df.groupby("draw")["y" + suf].apply(
        pyn.geo.spectral_diff, dt, order=2
    )
...
def synthetic_traj(
    ...
    if not spectral_diff:
        ds["x"] = (
            ds["uo"]
            .cumulative_integrate("time", datetime_unit="s")
    ...
    if spectral_diff:
        ...
        if all_comp_pos_acc:
            for suf in ["_low", "_ni", "_2", "_1"]:
                df = df.groupby("draw").apply(
                    pyn.geo.compute_accelerations,
                    from_=("xy", "x" + suf, "y" + suf),
                    names=("ax" + suf, "ay" + suf, "Axy" + suf),
                )
...
def irregular_time_sampling(
    ...
    if "x_noise" in ds:
        ds_off_noise = ds.interp(time=time_off, method="nearest")[
            ["x_noise", "y_noise"]
        ]
...
def noise_irregular_sampling(
    ...
    # IRREGULAR TIME SAMPLING
    ds = irregular_time_sampling(
        ds, offset_type, t, dt, file, istart, time_uniform=time_uniform
    )
...
def add_velocity_acceleration(ds, suffix="", method="pyn", groupby="draw"):
    ...
    if method == "spectral":
        ...
        df = df.groupby(groupby).apply(
            pyn.geo.compute_accelerations,
            from_=("xy", "x" + suffix, "y" + suffix),
            names=("ax" + suffix, "ay" + suffix, "Axy" + suffix),
        )

        df = df.groupby(groupby).apply(
            pyn.geo.compute_accelerations,
            from_=("velocities", "u" + suffix, "v" + suffix),
            names=("au" + suffix, "av" + suffix, "Auv" + suffix),
        )
...
def ratio_ms(
    DF, true_key="True_2h", var=["x", "y", "u", "v", "ax", "ay", "X", "U", "Axy"]
):
    DF = DF.copy()
    dft = DF[true_key]
    dfms = pd.DataFrame(index=DF.keys(), columns=var)
