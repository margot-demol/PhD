...
def plot_autocorrelations(ds, tau_max=100, suffixes_ls ={'':'solid'}, suffixes_leg={'':''}, title=''):
    ...
    for suf in suffixes_ls : 
        ds["xx"+suf].where(ds.lags < tau_max).plot.line(x="lags", ax=ax, ls = suffixes_ls[suf]);  
    ax.grid()
    ax.set_title("")
    ax.set_ylabel(r"$C_x$")
    ...
    if 'coru_inf' in list(ds.keys()):
        ds.coru_inf.where(ds.lags < tau_max).plot.line(x="lags",  color='k',ls=':',ax=ax, add_legend=False);
    for suf in suffixes_ls : 
        ds["uu"+suf].where(ds.lags < tau_max).plot.line(x="lags", ax=ax, ls = suffixes_ls[suf],add_legend=False);  
    ax.grid()
    ax.set_title("")
    ax.set_ylabel(r"$C_u$")
    ax.set_xlabel(r"$\tau$ [days]")
    ...
    if 'cora_inf' in list(ds.keys()):
        ds.cora_inf.where(ds.lags < tau_max).plot.line(x="lags",  color='k',ls=':',ax=ax, add_legend=False);
    for suf in suffixes_ls : 
        ds["aa"+suf].where(ds.lags < tau_max).plot.line(x="lags", ax=ax, ls = suffixes_ls[suf],add_legend=False);  
    ax.grid()
    ax.set_title("")
    ax.set_ylabel(r"$C_a$")
    ax.set_xlabel(r"$\tau$ [days]")
...
def plot_psd(ds, suffixes_ls ={'':'solid'}, suffixes_leg={'':''}, title='') : 
    ...
    # Vertical limitation
    for ax in axs :
        if ds.T.size== 1: ax.axvline(x = 1/ds.T, color=colors[0], ls='--', label='1/T')
        else:
