...
def get_eras_one_obs(ds_obs, dt=(-12,13), only_matchup_time = True):
    ...
    try : 
        ...
        ds_eras = ds_eras.rename({v:v.replace('tauu','taue') for v in ds_eras if 'tauu' in v}).rename({v:v.replace('tauv','taun') for v in ds_eras if 'tauv' in v})#NEW
    except : 
        assert False, ds_obs.__site_id
    

    # interplate over the trajectoire
    ds_traj = ds_eras.interp(es_time = ds_obs.drifter_time, es_lon = drifter_lon, es_lat = ds_obs.drifter_lat).rename({v: "traj_"+v for v in ds_eras}).drop(['drifter_lon', 'drifter_lat','es_time','es_lon','es_lat','lat','lon','time'])
    
    #at drifter matchup
    ds_drifter_matchup = ds_traj.isel(site_obs = site_matchup_indice).drop(list(ds_traj.coords.keys())).rename({v: v.replace('traj_', 'drifter_matchup_') for v in ds_traj})#NEW

    #interpolate on the box
    if only_matchup_time : 
        ds_box = ds_eras.interp(es_time = _drifter_time, es_lon = box_lon, es_lat = ds_obs.box_lat).rename({v: "box_"+v for v in ds_eras}).drop(['es_lon','es_lat','es_time','lat','lon','time'])
    else :
        ds_box = ds_eras.interp( es_lon = box_lon, es_lat = ds_obs.box_lat).rename({v: "box_"+v for v in ds_eras}).drop(['es_lon','es_lat','lat','lon','time'])
    ...
    ds_drifter = ds_eras.interp(es_lon = _drifter_lon, es_lat = _drifter_lat).drop(['es_lon', 'es_lat']).rename({v: "drifter_temp_"+v for v in ds_eras})
    ...
    ds = (xr.merge([ds_traj, ds_drifter_matchup, ds_box, ds_drifter, ds_alti])).reset_index("es_time").reset_coords(["drifter_time", "drifter_x", "drifter_y", 'es_time_'])#NEW
...
def compute_eras(ds, dt, only_matchup_time = True):
    ...
    """
    #attrs
    ds_eras.e5_alti_matchup_taue.attrs={'long_name':"era5 eastward wind stress interpolated on the altimeter's matchup", 'units':'Pa'}
    ds_eras.e5_alti_matchup_taun.attrs={'long_name':"era5 northward wind stress interpolated on the altimeter's matchup", 'units':'Pa'}

    ds_eras.e5_drifter_matchup_taue.attrs={'long_name':"era5 eastward wind stress interpolated on the drifter's matchup", 'units':'Pa'}
    ds_eras.e5_drifter_matchup_taun.attrs={'long_name':"era5 northward wind stress interpolated on the drifter's matchup", 'units':'Pa'}

    ds_eras.e5_box_taue.attrs={'long_name':"era5 eastward wind stress interpolated on the box at the matchup time or several times (depending on the dimension)", 'units':'Pa'}
    ds_eras.e5_box_taun.attrs={'long_name':"era5 northward wind stress interpolated on the box at the matchup time or several times (depending on the dimension)", 'units':'Pa'}

    ds_eras.e5_drifter_temp_taue.attrs={'long_name':"era5 eastward wind stress interpolated on the drifter's matchup position for several times (static but temporal variation)", 'units':'Pa'}
    ds_eras.e5_drifter_temp_taun.attrs={'long_name':"era5 northward wind stress interpolated on the drifter's matchup position for several times (static but temporal variation)", 'units':'Pa'}

    ds_eras.e5_traj_taue.attrs={'long_name':"era5 eastward wind stress interpolated on the drifter's trajectory", 'units':'Pa'}
    ds_eras.e5_traj_taun.attrs={'long_name':"era5 northward wind stress interpolated on the drifter's trajectory", 'units':'Pa'}


    ds_eras.es_alti_matchup_taue.attrs={'long_name':"erastar eastward wind stress interpolated on the altimeter's matchup", 'units':'Pa'}
    ds_eras.es_alti_matchup_taun.attrs={'long_name':"erastar northward wind stress interpolated on the altimeter's matchup", 'units':'Pa'}

    ds_eras.es_drifter_matchup_taue.attrs={'long_name':"erastar eastward wind stress interpolated on the drifter's matchup", 'units':'Pa'}
    ds_eras.es_drifter_matchup_taun.attrs={'long_name':"erastar northward wind stress interpolated on the drifter's matchup", 'units':'Pa'}

    ds_eras.es_box_taue.attrs={'long_name':"erastar eastward wind stress interpolated on the box at the matchup time", 'units':'Pa'}
    ds_eras.es_box_taun.attrs={'long_name':"erastar northward wind stress interpolated on the box at the matchup time", 'units':'Pa'}

    ds_eras.es_drifter_temp_taue.attrs={'long_name':"erastar eastward wind stress interpolated on the drifter's matchup position for several times (static but temporal variation)", 'units':'Pa'}
    ds_eras.es_drifter_temp_taun.attrs={'long_name':"erastar northward wind stress interpolated on the drifter's matchup position for several times (static but temporal variation)", 'units':'Pa'}

    ds_eras.es_traj_taue.attrs={'long_name':"erastar eastward wind stress interpolated on the drifter's trajectory", 'units':'Pa'}
    ds_eras.es_traj_taun.attrs={'long_name':"erastar northward wind stress interpolated on the drifter's trajectory", 'units':'Pa'}
