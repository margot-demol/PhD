...
def get_aviso_one_obs(ds_obs, dt=(-1,2), only_matchup_time = True):
    ...
    _drop = ['crs','lat_bnds','lon_bnds','ugosa', 'err_ugosa','vgosa',
             'err_vgosa','ugos','vgos','flag_ice','tpa_correction','nv']
    try : 
    ...
    ds_drifter = ds_aviso.interp(aviso_lon = _drifter_lon, aviso_lat = _drifter_lat).drop(['aviso_lon', 'aviso_lat']).rename({v: "aviso_drifter_temp_"+v for v in ds_aviso})
    ...
    try:
        ...
        ds_traj['aviso_traj_g_grad_x']= ds_box['aviso_box_g_grad_x'].interp(box_x=ds_traj.drifter_x, box_y=ds_traj.drifter_y)
        ds_traj['aviso_traj_g_grad_y']= ds_box['aviso_box_g_grad_y'].interp(box_x=ds_traj.drifter_x, box_y=ds_traj.drifter_y)
        #adt
        ds_box['aviso_box_adt_g_grad_x']= g * ds_box['aviso_box_adt'].differentiate('box_x')
        ds_box['aviso_box_adt_g_grad_y']= g * ds_box['aviso_box_adt'].differentiate('box_y')
        ds_traj['aviso_traj_adt_g_grad_x']= ds_box['aviso_box_adt_g_grad_x'].interp(box_x=ds_traj.drifter_x, box_y=ds_traj.drifter_y)
        ds_traj['aviso_traj_adt_g_grad_y']= ds_box['aviso_box_adt_g_grad_y'].interp(box_x=ds_traj.drifter_x, box_y=ds_traj.drifter_y)
        ...
        try:
            ...
            ds_traj['aviso_traj_g_grad_y']= ds_box['aviso_box_g_grad_y'].interp(box_x=ds_traj.drifter_x, box_y=ds_traj.drifter_y)
            #adt
            ds_box['aviso_box_adt_g_grad_x']= g * ds_box['aviso_box_adt'].compute().differentiate('box_x')
            ds_box['aviso_box_adt_g_grad_y']= g * ds_box['aviso_box_adt'].compute().differentiate('box_y')
            ds_traj['aviso_traj_adt_g_grad_x']= ds_box['aviso_box_adt_g_grad_x'].interp(box_x=ds_traj.drifter_x, box_y=ds_traj.drifter_y)
            ds_traj['aviso_traj_adt_g_grad_y']= ds_box['aviso_box_adt_g_grad_y'].interp(box_x=ds_traj.drifter_x, box_y=ds_traj.drifter_y)
        except:
    ...
    #adt
    ds_alti['aviso_alti_matchup_adt_g_grad_x']= ds_box['aviso_box_adt_g_grad_x'].sel(box_x=0, box_y=0)
    ds_alti['aviso_alti_matchup_adt_g_grad_y']= ds_box['aviso_box_adt_g_grad_y'].sel(box_x=0, box_y=0)
    
    #at drifter matchup
    ds_drifter_matchup = ds_traj[[v for v in ds_traj if 'grad' in v]].isel(site_obs = site_matchup_indice)
    ds_drifter_matchup = ds_drifter_matchup.rename({v: v.replace('traj_', 'drifter_matchup_') for v in ds_drifter_matchup}).drop(list(ds_drifter_matchup.coords.keys()))#NEW
    
    # convert to dataset and massage in order to concatenate properly in get_aviso
    ds = (xr.merge([ds_traj, ds_drifter_matchup, ds_box, ds_drifter, ds_alti])).reset_index("aviso_time").reset_coords(["drifter_time", "drifter_x", "drifter_y", 'aviso_time_'])
...
def compute_aviso_sla(ds, dt=(-1,2), only_matchup_time = True):
    ...
    """
    #attrs
    ds_aviso.aviso_alti_matchup_err_sla.attrs={'description':"sla error interpolated on the altimeter's matchup",
                                               'long_name':r"$err\eta_{altimatchup}$",
                                               'units':'m'}
    ds_aviso.aviso_alti_matchup_sla.attrs={'description':"sla interpolated on the altimeter's matchup",
                                           'long_name':r"$\eta_{altimatchup}$",
                                           'units':'m'}
    ds_aviso.aviso_alti_matchup_adt.attrs={'description':"adt interpolated on the altimeter's matchup",
                                           'long_name':r"$adt_{altimatchup}$",
                                           'units':'m'}
    ds_aviso.aviso_alti_matchup_g_grad_x.attrs={'description':"along track sla gradient term interpolated on the altimeter's matchup",
                                                'long_name':r"$g\partial_x\eta_{altimatchup}$",
                                                'units':r'$m.s^{-2}$'}
    ds_aviso.aviso_alti_matchup_g_grad_y.attrs={'description':"cross track sla gradient term interpolated on the altimeter's matchup",
                                                'long_name':r"$g\partial_y\eta_{altimatchup}$",
                                                'units':r'$m.s^{-2}$'}
    ds_aviso.aviso_alti_matchup_adt_g_grad_x.attrs={'description':"along track adt gradient term interpolated on the altimeter's matchup",
                                                'long_name':r"$g\partial_xadt_{altimatchup}$",
                                                'units':r'$m.s^{-2}$'}
    ds_aviso.aviso_alti_matchup_adt_g_grad_y.attrs={'description':"cross track adt gradient term interpolated on the altimeter's matchup",
                                                'long_name':r"$g\partial_yadt_{altimatchup}$",
                                                'units':r'$m.s^{-2}$'}


    ds_aviso.aviso_box_err_sla.attrs={'description':"sla error interpolated on the box at the matchup time or for several times (depending on the dimension)",
                                      'long_name':r"$err\eta_{box}$",
                                      'units':'m'}
    ds_aviso.aviso_box_sla.attrs={'description':"sla interpolated on the box at the matchup time or for several times (depending on the dimension)",
                                  'long_name':r"$\eta_{box}$",
                                  'units':'m'}
    ds_aviso.aviso_box_g_grad_x.attrs={'description':"along track sla gradient term interpolated on the box at the matchup time or for several times (depending on the dimension)",
                                       'long_name':r"$g\partial_x\eta_{box}$",
                                       'units':r'$m.s^{-2}$'}
    ds_aviso.aviso_box_g_grad_y.attrs={'description':"cross track sla gradient term interpolated on the box at the matchup time or for several times (depending on the dimension)",
                                       'long_name':r"$g\partial_y\eta_{box}$",
                                       'units':r'$m.s^{-2}$'}
    ds_aviso.aviso_box_adt.attrs={'description':"adt interpolated on the box at the matchup time or for several times (depending on the dimension)",
                                  'long_name':r"$adt_{box}$",
                                  'units':'m'}
    ds_aviso.aviso_box_adt_g_grad_x.attrs={'description':"along track adt gradient term interpolated on the box at the matchup time or for several times (depending on the dimension)",
                                       'long_name':r"$g\partial_xadt_{box}$",
                                       'units':r'$m.s^{-2}$'}
    ds_aviso.aviso_box_adt_g_grad_y.attrs={'description':"cross track adt gradient term interpolated on the box at the matchup time or for several times (depending on the dimension)",
                                       'long_name':r"$g\partial_yadt_{box}$",
                                       'units':r'$m.s^{-2}$'}


    ds_aviso.aviso_traj_err_sla.attrs={'description':"sla error interpolated on the drifter's trajectory",
                                       'long_name':r"$err\eta_{traj}$",
                                       'units':'m'}
    ds_aviso.aviso_traj_sla.attrs={'description':"sla interpolated on the drifter's trajectory",
                                   'long_name':r"$\eta_{traj}$",
                                   'units':'m'}
    ds_aviso.aviso_traj_g_grad_x.attrs={'description':"along track sla gradient term interpolated on the drifter's trajectory",
                                        'long_name':r"$g\partial_x\eta_{traj}$",
                                        'units':r'$m.s^{-2}$'}
    ds_aviso.aviso_traj_g_grad_y.attrs={'description':"cross track sla gradient term interpolated on the drifter's trajectory",
                                        'long_name':r"$g\partial_y\eta_{traj}$",
                                        'units':r'$m.s^{-2}$'}
    ds_aviso.aviso_traj_adt.attrs={'description':"adt interpolated on the drifter's trajectory",
                                   'long_name':r"$adt_{traj}$",
                                   'units':'m'}
    ds_aviso.aviso_traj_adt_g_grad_x.attrs={'description':"along track adt gradient term interpolated on the drifter's trajectory",
                                        'long_name':r"$g\partial_xadt_{traj}$",
                                        'units':r'$m.s^{-2}$'}
    ds_aviso.aviso_traj_adt_g_grad_y.attrs={'description':"cross track adt gradient term interpolated on the drifter's trajectory",
                                        'long_name':r"$g\partial_yadt_{traj}$",
                                        'units':r'$m.s^{-2}$'}

    
    ds_aviso.aviso_drifter_matchup_g_grad_x.attrs={'description':"along track sla gradient term interpolated on the drifter's matchup", 
                                                   'long_name':r"$g\partial_x\eta_{driftermatchup}$",
                                                   'units':r'$m.s^{-2}$'}
    ds_aviso.aviso_drifter_matchup_g_grad_y.attrs={'description':"cross track sla gradient term interpolated on the drifter's matchup",
                                                   'long_name':r"$g\partial_y\eta_{altimetermatchup}$",
                                                   'units':r'$m.s^{-2}$'}
    ds_aviso.aviso_drifter_matchup_adt_g_grad_x.attrs={'description':"along track adt gradient term interpolated on the drifter's matchup", 
                                                   'long_name':r"$g\partial_xadt_{driftermatchup}$",
                                                   'units':r'$m.s^{-2}$'}
    ds_aviso.aviso_drifter_matchup_adt_g_grad_y.attrs={'description':"cross track adt gradient term interpolated on the drifter's matchup",
                                                   'long_name':r"$g\partial_yadt_{altimetermatchup}$",
                                                   'units':r'$m.s^{-2}$'}
    
    ds_aviso.aviso_drifter_temp_err_sla.attrs={'description':"sla error interpolated on the drifter's matchup position for several times (static but temporal variation)",
                                               'long_name':r"$err\eta_{driftertemporal}$",
                                               'units':'m'}
    ds_aviso.aviso_drifter_temp_sla.attrs={'description':"sla interpolated on the drifter's matchup position for several times (static but temporal variation)",
                                           'long_name':r"$\eta_{driftertemporal}$",
                                           'units':'m'}
    ds_aviso.aviso_drifter_temp_adt.attrs={'description':"adt interpolated on the drifter's matchup position for several times (static but temporal variation)",
