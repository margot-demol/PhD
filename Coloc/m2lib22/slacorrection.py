...
def load_corr(ts, CMEMS_dir, suffix="alti_", rkwargs=None, **kwargs):
    ...
    **kwargs: dict
    ...
    #LOAD
    try : 
        files = glob(os.path.join(CMEMS_dir, f"{ts.year}/{ts.month:02d}/*{ts.year}{ts.month:02d}{ts.day:02d}*.nc"))
        ds = (xr.load_dataset(files[0], **rkwargs).sel(**kwargs))
    except :
        assert False, ts
        
    ds = ds.rename({'longitude':'lon', 'latitude':'lat', 'ocean_tide':'oceantide'})#NEW
...
def get_corr_one_obs(ds_obs):
    ...
    if NaT : 
        try : 
            _ds_corr = load_corr(ts, CMEMS_dir, suffix="alti_",
                        rkwargs={'drop_variables':_drop},
                        time =Tn, method='nearest', tolerance = pd.Timedelta(np.timedelta64(1, "us")))
        except:
            try:
                _ds_corr = xr.concat([load_corr(ts, CMEMS_dir, suffix="alti_",rkwargs={'drop_variables':_drop}),
                                   ...
                                   dim="alti_time",).sortby('alti_time').sel(alti_time=Tn, method='nearest', tolerance = pd.Timedelta(np.timedelta64(1, "us")))
            except:
    ...
    ds_corr['alti_adt']= (ds_obs.alti_sla_denoised + ds_corr.alti_mdt).assign_attrs({'long_name': 'Absolute Dynamic Topography', 'description':'alti_sla_denoised+alti_mdt', 'units':'m'})
    ...
    ds_corr['alti_adt_oceantide']=(ds_corr.alti_adt+ds_corr.alti_oceantide).assign_attrs({'long_name': 'Adt+Ocean tide', 'description':'alti_sla_denoised+alti_mdt+alti_oceantide','units':'m'})#NEW
    ds_corr['alti_adt_oceantide_dac']=(ds_corr.alti_adt+ds_corr.alti_oceantide+ds_corr.alti_dac).assign_attrs({'long_name': 'Adt+Ocean tide+dac', 'description':'alti_sla_denoised+alti_mdt+alti_oceantide+dac','units':'m'})#NEW
...
def compute_corr_sla(ds):
    ...
    ds_corr.alti_mdt_g_grad_x.attrs = {'long_name':r'$g\partial_xmdt$', 
                                    ...
                                    'units':r'$m.s^{-2}$'}
    ds_corr.alti_oceantide_g_grad_x.attrs = {'long_name':r'$g\partial_xoceantide$', #NEW oceantide ocean_tide
                                     'description':"along track ocean tide model gradient term ",
                                    'units':r'$m.s^{-2}$'}
    ds_corr.alti_adt_oceantide_g_grad_x.attrs = {'long_name':r'$g\partial_x(adt+oceantide)$', #NEW 
                                     'description':"along track adt + ocean tide gradient term ",
                                    'units':r'$m.s^{-2}$'}
    ds_corr.alti_adt_oceantide_dac_g_grad_x.attrs = {'long_name':r'$g\partial_x(adt+oceantide+dac)$', #NEW 
