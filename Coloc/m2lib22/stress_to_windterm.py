...
def compute_wd_from_stress(ds, list_wd_srce_suffix=list_wd_srce_suffix, list_func=list_func, list_func_suffix=list_func_suffix, east_north=False):
    ...
    for i in range(len(list_func)):
        ...
        for src in list_wd_srce_suffix :
            ...
            _ds[src+'_'+suf+'_alti_wd_x'].attrs = {'long_name': r"$(-\frac{1}{\rho}\partial_z\tau_x)_{altimatchup}$ from "+suf,
                                                   'description':"along track wind related term interpolated at the altimeter's matchup from "+source+fonction,
                                                   'units':r'$m.s^{-2}$'}
            _ds[src+'_'+suf+'_alti_wd_y'].attrs = {'long_name': r' $(-\frac{1}{\rho}\partial_z\tau_y)_{altimatchup}$ from '+suf,
                                                   'description':"cross track wind related term interpolated at the altimeter's matchup from "+source+fonction,
                                                   'units':r'$m.s^{-2}$'}
            _ds[src+'_'+suf+'_drifter_wd_x'].attrs = {'long_name':r'$(-\frac{1}{\rho}\partial_z\tau_x)_{driftermatchup}$ from '+suf,
                                                   'description':"along track wind related term interpolated at the drifter's matchup from "+source+fonction,
                                                   'units':r'$m.s^{-2}$'}
            _ds[src+'_'+suf+'_drifter_wd_y'].attrs = {'long_name':r'$(-\frac{1}{\rho}\partial_z\tau_y)_{driftermatchup}$ from '+suf,
            ...
            if east_north:
                ...
                #attrs
                _ds[src+'_'+suf+'_alti_wd_e'].attrs = {'long_name':r"$(-\frac{1}{\rho}\partial_z\tau_e)_{altimatchup}$ from "+suf,
                                                   'description':"eastward wind related term interpolated at the altimeter's matchup from "+source+fonction,
                                                   'units':r'$m.s^{-2}$'}
                _ds[src+'_'+suf+'_alti_wd_n'].attrs = {'long_name':r'$(-\frac{1}{\rho}\partial_z\tau_n)_{altimatchup}$ from '+suf,
                                                   'description':"northward wind related term interpolated at the altimeter's matchup from "+source+fonction,
                                                   'units':r'$m.s^{-2}$'}
                _ds[src+'_'+suf+'_drifter_wd_e'].attrs = {'long_name':r'$(-\frac{1}{\rho}\partial_z\tau_e)_{driftermatchup}$ from '+suf,
                                                   'description':"eastward wind related term interpolated at the drifter's matchup from "+source+fonction,
                                                   'units':r'$m.s^{-2}$'}
                _ds[src+'_'+suf+'_drifter_wd_n'].attrs = {'long_name':r'$(-\frac{1}{\rho}\partial_z\tau_n)_{driftermatchup}$ from '+suf,
