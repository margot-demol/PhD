...
def build_dataset(nc, 
    ...
    else : ds["alti_denoised_g_grad_x"] = xr.full_like(ds["alti_g_grad_x"], np.nan)
    ...
    #attrs
    ds.drifter_x.attrs={'description':"drifter's along track direction x position on the box", "units":"m"}
    ds.drifter_y.attrs={'description':"drifter's cross track direction y position on the box", "units":"m"}
    ds.drifter_lon.attrs={'description':"drifter's longitude", "units":"°"}
    ds.drifter_lat.attrs={'description':"drifter's latitude", "units":"°"}

    ds.alti_x.attrs={'description':"altimeter's along track direction x position on the box", "units":"m"}
    ds.alti_y.attrs={'description':"altimeter's cross track direction y position on the box", "units":"m"}

    ds.box_x.attrs={'description':"along track direction x coordinate of the box", "units":"m"}
    ds.box_y.attrs={'description':"cross track direction y coordinate of the box", "units":"m"}

    ds.drifter_vx.attrs={'description':"drifter's along track direction velocity on the box", "units":r"$m.s^{-1}$", 'long_name':'u'}
    ds.drifter_vy.attrs={'description':"drifter's cross track direction velocity on the box", "units":r"$m.s^{-1}$", 'long_name':'v'}
    ds.drifter_ve.attrs={'description':"drifter's eastward velocity", "units":r"$m.s^{-1}$", 'long_name':r'$u_{eastward}$'}
    ds.drifter_vn.attrs={'description':"drifter's northward velocity", "units":r"$m.s^{-1}$", 'long_name':r'$v_{northward}$'}

    ds.drifter_acc_x.attrs={'description':"drifter's along track direction acceleration on the box", "units":r"$m.s^{-2}$", 'long_name':r'$d_tu$'}
    ds.drifter_acc_y.attrs={'description':"drifter's cross track direction acceleration on the box", "units":r"$m.s^{-2}$", 'long_name':r'$d_tv$'}

    ds.drifter_coriolis_x.attrs={'description':"along track coriolis term from drifter", "units":r"$m.s^{-2}$", 'long_name':r'$-fv$'}
    ds.drifter_coriolis_y.attrs={'description':"cross track coriolis term from drifter", "units":r"$m.s^{-2}$", 'long_name':r'$fu$'}

    ds.drifter_time.attrs={'description':"drifter's trajectory time measurements"}

    ds.alti_g_grad_x.attrs={'description':"cross track sla gradient term from the altimeter", "units":r"$m.s^{-2}$", 'long_name':r'$g\partial_x\eta$'}
    ds.alti_denoised_g_grad_x.attrs={'description':"denoised cross track sla gradient term from the altimeter", "units":r"$m.s^{-2}$", 'long_name':r'$g\partial_x\eta$'}

    ds.box_lat.attrs={'description':"latitudes of box points", "units":"°",}
    ds.box_lon.attrs={'description':"longitudes of box points", "units":"°",}
    ds.box_latc.attrs={'description':"latitude of the center of the track, the altimeter's matchup point", "units":"°",}
    ds.box_lonc.attrs={'description':"longitude of the center of the track, the altimeter's matchup point", "units":"°",}
    ds.box_phi.attrs={'description':"angle between the track and the longitude axis", "units":"°",}
    ds.box_theta_lat.attrs={'description':"local angle between the track and the longitude axis for box's points", "units":"°",}
    ds.box_theta_lon.attrs={'description':"local angle between the track and the latitude axis for box's points", "units":"°",}

    ds.drifter_theta_lat.attrs={'description':"local angle between the track and the longitude axis along the drifter's trajectory", "units":"°",}
    ds.drifter_theta_lon.attrs={'description':"local angle between the track and the latitude axis along the drifter's trajectory", "units":"°",}
    ds.f.attrs={'description':"coriolis frequency at the altimeter's matchup point", "units":r"$s^{-1}$",}
